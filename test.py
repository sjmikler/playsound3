...
def mockMciSendStringW(command, buf, bufLen, bufStart):
    ...
    if command.startswith("close "):
        ...
        sawClose = True
        testCase.assertIn(
            originalMCISendStringW(command, buf, bufLen, bufStart), [0, 263]
        )  # 263 indicates it's not opened or not recognized. It's fine.
        return 0

    if command.endswith(" wait"):
        sleep(expectedDuration)

    if command.startswith("open ") or command.startswith("play "):
        testCase.assertIn(
            originalMCISendStringW(command, buf, bufLen, bufStart), [0, 306]
...
class PlaysoundTests(unittest.TestCase):
    ...
    def helper(self, file, approximateDuration, block=True):
        ...
        self.assertTrue(
            approximateDuration - durationMarginLow <= duration <= approximateDuration + duratingMarginHigh,
    ...
    def testMissing(self):
        ...
        for sub in ["not", "fakefile.wav"]:
            self.assertIn(
                sub, message, '"{}" was expected in the exception message, but instead got: "{}"'.format(sub, message)
            )
